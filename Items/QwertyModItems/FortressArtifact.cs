...
{
	public class FortressArtifact : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			Tooltip.SetDefault("Not Equippable"
				...
				+ $"\n'This box thing looks like it could sell for a fortune'");
			...
			Tooltip.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Polish), "Nie mozna zalozyc"
				...
				+ $"\n'Te pudelko wyglada na cos co mozna sprzedac za fortune'");
	...
	//	{
	//		if (!ModLoader.TryGetMod("QwertyMod", out var QwertyMod))
	//		{
	//			return;
	//		}
	//		if (!ModContent.TryFind("QwertyMod/NanoDart", out ModItem NanoDart)) return;
    //      {
	//			CreateRecipe()
	//		.AddIngredient(ModContent.ItemType<FortressBrick>())
	//		.AddIngredient(ModContent.TryGetMod(QwertyMod).ItemType("MadnessFragment")), 4)
